 ...
 * @return
 */
int kwSocketClient::write(char *buff, int len)
{
 ...
 * @return
 */
int kwSocketClient::read(char *buff, size_t maxlen)
{
  return ::read(m_SocketFd, buff, maxlen);
}


void kwSocketClient::deInit()
{
  close(m_SocketFd);
  m_SocketFd = -1;
}
 ...
 * @return
 */
kwValString kwSocketClient::sendRequest(kwValString* request)
{
